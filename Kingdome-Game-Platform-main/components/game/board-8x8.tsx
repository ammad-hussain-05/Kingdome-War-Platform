...
function FireworksCanvas() {
  ...
  useEffect(() => {
    ...
    const particles: P[] = [];
    const colors = ["#FFD700","#FF6B35","#E8DFC0","#C8A96E","#FF4500","#FFA500","#FFFFFF","#FFB6C1","#90EE90"];
    const burst = (x: number, y: number) => {
      for (let i = 0; i < 70; i++) {
        const angle = Math.random() * Math.PI * 2;
        const speed = Math.random() * 9 + 2;
        particles.push({ x, y, vx:Math.cos(angle)*speed, vy:Math.sin(angle)*speed-3, alpha:1, color:colors[Math.floor(Math.random()*colors.length)], size:Math.random()*4+1.5 });
      }
    };
    let frame = 0;
    const loop = () => {
      ...
      if (frame % 22 === 0) burst(Math.random() * canvas.width, Math.random() * canvas.height * .55);
      for (let i = particles.length - 1; i >= 0; i--) {
        const p = particles[i];
        p.x += p.vx; p.y += p.vy; p.vy += .18; p.alpha -= .017;
        if (p.alpha <= 0) { particles.splice(i, 1); continue; }
        ctx.save(); ctx.globalAlpha = p.alpha; ctx.fillStyle = p.color;
        ctx.beginPath(); ctx.arc(p.x, p.y, p.size, 0, Math.PI * 2); ctx.fill(); ctx.restore();
      }
      frame++;
      if (frame < 350) requestAnimationFrame(loop);
